...
ROOT.Rip5 = [ROOT.Mother '\Processed Data\ripples_mat\R5_cell'];
ROOT.Fig = [ROOT.Mother '\Processed Data\ripples_mat\ProfilingSheet\R20_sub'];
...
RipplesTable = readtable([ROOT.Save '\RipplesTable_' thisRegion2 '_RDIs_UV_cell_HeteroIn.xlsx']);
...
NS_perm = load([ROOT.Rip5 ['\' thisRegion2 '-' thisRip.ID{1} '_UV.mat']],'thisUnits','RDI_L','RDI_R','RDI_C');
...
function FRMaps = LoadFRMap(ROOT,clusters)
FRMaps = [];
for cl=1:size(clusters.ID,1)
    try
        thisField = table;
        temp=[];
        thisRSID = [clusters.ID{cl}(1:3) '-' clusters.ID{cl}(5:6)];
        thisTTID = num2str(str2double(clusters.ID{cl}(8:9)));
        thisCLID =  clusters.ID{cl}(11:12);
        thisFLID =  jmnum2str(str2double(clusters.ID{cl}(14:end)),2);

        if length(clusters.ID{cl})<14
