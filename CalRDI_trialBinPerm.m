function [c,d,rand_d,p] = CalRDI_trialBinPerm(clusterID,ROOT,Behav,Spike)
...
%                 pf(:,1) = getFRfields(Spk_processed.skaggsMap1D{1,1},crit_pf);
            ...
            for cond_iter = 1 : 3
                ...
                id2 = sample_idx_cond{cond_iter}(:,2);
                sample2 = nanmean(thisFieldMap1D_trial(find(pf(:,1)),id2),1); %% cxt2의 mean field firing rate 모음
                
                
                if DoRand
                parfor rand_iter = 1:NumRand
                    thisFieldMap1D_trial_rand=[];
                    for s = 1:size(thisFieldMap1D_trial,2)
                    thisFieldMap1D_trial_rand(:,s) = thisFieldMap1D_trial(randperm(size(thisFieldMap1D_trial,1)),s);
                    end
                    sample = nanmean(thisFieldMap1D_trial_rand(find(pf(:,1)),:),1); %% cxt1의 mean field firing rate 모음
                    sample_sum = [sample]; %% 각 trial에서의 mean firing rate을 한 줄로 모으기
